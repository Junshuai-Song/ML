...
缺点：
（1）貌似只能做二分类 ？ 但是多次二分类组合一下也可以做成多类别？

补充一个神经网络缺点：
（1）由于模型的不可解释性，其分类会依据训练集产生一些很奇怪的特征，就算很少数据也会有轻微的“类似过拟合”现象；
    即：在一个螺旋图中，其会在螺旋的缝隙间导出一个扇形，分成不同的类别，而这是与我们的目标不符的。
（2）太过依据样本，如螺旋图案的分类，样本覆盖范围之外的部分，区分率大大下降。
    这就要求，使用神经网络的前提：样本尽可能覆盖所有情况。即神经网络对未知情况的预测十分不准确！
    所以对于深度学习，需要不断迭代，尽可能模拟所有情况，如围棋大战。
（3）异常点对神经网络的训练影响比较大。
（4）问题： 神经网络一定会收敛吗？
